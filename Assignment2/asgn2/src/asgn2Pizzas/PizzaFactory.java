...
//	margherita(��PZM��), vegetarian (��PZV��) or meat lovers (��PZL��). 
	...
	public static Pizza getPizza(String pizzaCode, int quantity, LocalTime orderTime, LocalTime deliveryTime) throws PizzaException{
		...
		Pizza NewOrderPizza = null;
		if(pizzaCode.equals("PZM")){
			NewOrderPizza = new MargheritaPizza(quantity, orderTime, deliveryTime);
		}
		else if(pizzaCode.equals("PZV")){
			NewOrderPizza = new VegetarianPizza(quantity, orderTime, deliveryTime);
		}
		else if(pizzaCode.equals("PZL")){
			NewOrderPizza = new MeatLoversPizza(quantity, orderTime, deliveryTime);
		}
		else{
			throw new PizzaException("This pizza code is not valid");
		}
		return NewOrderPizza;
	}
