...
/**
 * A class to model a soccer team. The team contains a series of statistics
 * which are altered throughout a season depending on the results from their
 * matches.
 * 
 * @author Alan Woodley
 * @version 1.0
...
public class SoccerTeam implements SportsTeam, Comparable<SoccerTeam> {
	 ...
	 * 
	 * @param official
	 *            The official name of the soccer team.
	 * @param nick
	 *            The nick name of the soccer team.
	 * @throws TeamException
	 *             If either the official or nick are empty strings.
	 */
	public SoccerTeam(String official, String nick) throws TeamException {
		...
		if (official == null || official == "") {
			throw new TeamException("Official name is not valid");
		} else if (nick == null || nick == "") {
			throw new TeamException("nick name is not valid");
		} else if ((nick == null || nick == "") && (official == null || official == "")) {
			throw new TeamException("nick and Official names are not valid");
		}
	...
	/**
	 * Displays a String that provides details about the team. The format of the
	 * string is as follows (where - indicates a tab) :
	 * 
	 * official name - nick name - form - matches played - matches won - matches
	 * lost - matches drawn - season goals scores - season goals conceded -
	 * season goal difference - points
	 * 
	 */
	public void displayTeamDetails() {
		System.out.println(
				officialName + '\t' + nickName + '\t' + form + '\t' + (+matchesWon + matchesLost + matchesDrawn) + '\t'
	...
	/**
	 * Returns the goals conceded (goals scored by other teams against this
	 * team) by the team so far this season.
	 * 
	...
	/**
	 * Returns the goal difference (goals scored minus goals conceded) for the
	 * team across the course of a season.
	 * 
	 ...
	 * @throws TeamException
	 *             If the number of goals scored or conceded is an unrealistic
	 *             number (less than 0 or greater than 20).
	 */
	public void playMatch(int goalsFor, int goalsAgainst) throws TeamException {
		...
		if (goalsFor < 0 || goalsFor > 20) {
			throw new TeamException("The goals is not valid. The number of golas is less than 0 or greater than20");
		} else if (goalsAgainst < 0 || goalsAgainst > 20) {
			throw new TeamException("The goals is not valid. The number of golas is less than 0 or greater than20");
		}

		else if ((goalsFor < 0 || goalsFor > 20) && (goalsAgainst < 0 || goalsAgainst > 20)) {
			throw new TeamException(
					"The number of goals scored and conceded is not valid. The number of golas is less than 0 or greater than20");
		}

		goalsScoredSeason = goalsScoredSeason + goalsFor;
		goalsConcededSeason = goalsConcededSeason + goalsAgainst;

		// Win -> +3 for competition point , Loss -> + 0 , Draw -> + 1
		if (goalsFor > goalsAgainst) {
			form.addResultToForm(WLD.WIN);
			competitionPoints = competitionPoints + 3;
			...
			competitionPoints = competitionPoints + 1;
			matchesDrawn = matchesDrawn + 1;
		}
	 ...
	 * 
	 * A positive number is returned if the other team has more points than this
	 * team. A negative number is returned if this team has more points than the
	 * other team. If both teams have same number of points then: A positive
	 * number is returned if the other team has a greater goal difference than
	 * this team. A negative number is returned if this team has a greater goal
	 * difference than the other team. If both teams have the same number of
	 * points and the same goal difference then: A positive number is returned
	 * if the other team's official name occurs before this team alphabetically.
	 * A negative number is returned if this team official name occurs before
	 * the other team alphabetically. If teams have the same number of points,
	 * same goal difference, and same name then 0 is returned
	 * 
	 * @param other
	 *            The other team
