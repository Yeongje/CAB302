...
public class SoccerLeague implements SportsLeague {
	...
	private ArrayList<SoccerTeam> leag;
	 ...
	 * One that number of teams has been research no more teams can be added
	...
	public void registerTeam(SoccerTeam team) throws LeagueException {
		...
		if (offSeason == false) {
			throw new LeagueException("The league is already started");
		} else if (leag.size() >= requiredTeams) {
			throw new LeagueException("Now, enough teams in this league");
		}
		for (int i = 0; i < leag.size(); i++) {
			if (leag.get(i).getOfficialName().equals(team.getOfficialName())) { /////////
				throw new LeagueException("The team is already registered");
			}
		}
		leag.add(team);
	}
	...
	public void removeTeam(SoccerTeam team) throws LeagueException {
		...
		int a = 1; // Check whether the team is registered or not
		if (offSeason == true) {
			for (int i = 0; i < leag.size(); i++) {
				if (leag.get(i).getOfficialName().equals(team.getOfficialName())) {
					leag.remove(i);
			...
			throw new LeagueException("This season is not end");
		}
	...
	/**
	 * Starts a new season by reverting all statistics for each times to initial
	 * values.
	 * 
	 * @throws LeagueException
	...
	public void startNewSeason() throws LeagueException {
		...
		if (offSeason == false) {
			...
			throw new LeagueException("These registered teams is not enough");
		}

		offSeason = false; // Starts new season.

		for (int i = 0; i < leag.size(); i++) {

			leag.get(i).resetStats();
	...
	public void endSeason() throws LeagueException {
		...
		if (offSeason == true) {
			throw new LeagueException("The season is not started");
		}
		offSeason = true; // Ends the season
	}
	...
	public SoccerTeam getTeamByOfficalName(String name) throws LeagueException {
		...
		for (int i = 0; i < leag.size(); i++) {
			if (leag.get(i).getOfficialName().equals(name)) {
				return leag.get(i);
			}
		}

		throw new LeagueException("Cannot find the team by official name");
	}
	...
	public void playMatch(String homeTeamName, int homeTeamGoals, String awayTeamName, int awayTeamGoals)
		...
		if (homeTeamName.equals(awayTeamName)) {
			throw new LeagueException("The team name is same with the opposing team");
		}
		if (containsTeam(homeTeamName) == false) {
			throw new LeagueException("Home Team not found!");
		}
		if (containsTeam(awayTeamName) == false) {
			throw new LeagueException("Away Team not found!");
		}

		for (int i = 0; i < leag.size(); i++) {
			if (leag.get(i).getOfficialName().equals(homeTeamName)) {
				try {
					leag.get(i).playMatch(homeTeamGoals, awayTeamGoals);
				} catch (TeamException e) {
					throw new LeagueException(e.getMessage());
				}
			} else if (leag.get(i).getOfficialName().equals(awayTeamName)) {
				try {
					leag.get(i).playMatch(awayTeamGoals, homeTeamGoals);
				} catch (TeamException e) {
					throw new LeagueException(e.getMessage());
				}
			}
	...
	public SoccerTeam getTopTeam() throws LeagueException {
		...
		if (leag.size() < requiredTeams) {
			throw new LeagueException("The number of teams is zero or less than the required number of teams");
		}
